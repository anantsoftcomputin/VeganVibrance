    ...
    const fetchRelatedBlogs = async () => {
      ...
      try {
        const response = await ajaxCall(
          `/posts-category/?site_domain=theveganvibrance.com&category_name=${category}`,
          { method: "GET" }
        );
            ...
            <div ref={contentRef}>
              ...
              >
                <div className="flex flex-wrap items-center gap-3 text-sm">
                  <span className="px-3 py-1 rounded-full bg-gradient-to-r from-green-600 to-green-600 text-white font-medium">
                    {blog.category?.name}
                  </span>
